...
class densehrnet(nn.Module):
    def __init__(self, drop_rate=0, num_classes=2):
        ...
        self.getedge1 = nn.Sequential(
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True)
        )
        self.edge1_out = nn.Sequential(
        ...
        self.getedge2 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True)
        )
        self.edge2_out = nn.Sequential(
        ...
        self.getedge3 = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True)
        )
        self.edge3_out = nn.Sequential(
        ...
        self.getedge4 = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
            nn.ReLU(inplace=True)
        )
        self.edge4_out = nn.Sequential(
            nn.Conv2d(256, 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Sigmoid()
        )
        self.alledge = nn.Sequential(
            nn.Conv2d(480, 2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Sigmoid()
        )
        self.outconv = nn.Sequential(
            SEBlock('avg',256,256),
    ...
    def forward(self, x):
        ...
        x1_edge = self.getedge1(x1)
        x1_edge_out = self.edge1_out(x1_edge)
        # 2
        x2 = self.conv2(x1)
        x2 = self.denselayer2(x2)
        x2_edge = self.getedge2(x2)
        x2_edge_out = self.edge2_out(x2_edge)        
        # x2 = self.hrmodel(x2)
        # 3
        x3 = torch.cat([x1, x2], dim=1)
        x3_ = self.conv3(x3)
        x3_ = self.denselayer3(x3_)
        x3_edge = self.getedge3(x3_)
        x3_edge_out = self.edge3_out(x3_edge)
        # 4
        x4 = torch.cat([x2, x3_], dim=1)
        x4_ = self.conv4(x4)
        x4_ = self.denselayer4(x4_)
        x4_edge = self.getedge4(x4_)
        x4_edge_out = self.edge4_out(x4_edge)
        # 480
        edge = torch.cat([x1_edge,x2_edge,x3_edge,x4_edge],dim=1)
        edge_out = self.alledge(edge)
        # cat = torch.cat([x1,x2,x3,x4], dim=1)
        x5 = self.outconv(x4_)
#         return x5,x1_edge_out,x2_edge_out,x3_edge_out,x4_edge_out,edge_out
        return x5
