...
def ConfusionMatrix(numClass, imgPredict, Label):
    ...
    # 确保标签在正确的范围内
    Label = Label % numClass

    # 返回混淆矩阵
    mask = (Label >= 0) & (Label < numClass)
    label = numClass * Label[mask].astype(int) + imgPredict[mask]
...
#    ndvi = -(ndvi_pre[0] - ndvi_pre[1]) / (ndvi_pre[0] + ndvi_pre[1])
    ...
    max= 0
    maxi=0
    for i in range(int(ndvi_pre.min()*100),int(ndvi_max*100)):
        #  类别数目(包括背景)
        print(i/100.0)
        pre = r"/chenruipeng/GreenTide/test/1.tif"
        ndvi_pre = io.imread(pre)
        ...
        if round(score[4], 4)>max:
            max=round(score[4], 4)
            maxi = i
        print(round(score[4], 4))
        print(maxi)
        io.imsave(r"/chenruipeng/GreenTide/save_res2/rvi" + str(i)+'_f1='+str(round(score[2][1], 4)) + '.tif', y_pre)