...
from mdoel.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
...
model_urls = {
    ...
    'mae-vit-base': '/chenruipeng/RedTideDetection/ckpt/mae_pretrain_vit_base.pth',
    'zlp-vit-base': '/chenruipeng/RedTideDetection/ckpt/vit-b-checkpoint-1599.pth'
}
...
class Deeplabv3plus_vit(nn.Module):
    def __init__(self, backbone, num_classes=3, os=16, freeze_bn=False):
        ...
        self.conv2 = nn.Conv2d(3, 48, 1, bias=False)
        self.bn2 = BatchNorm2d(48)
        self.last_conv = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm2d(256),
                                       nn.ReLU(),
    ...
    def forward(self, input):
        x, low_level_features = self.vit_features(input)
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
        x4 = self.aspp4(x)
...
def Deeplabv3plus_vitbase(num_classes=2, image_size=64, os=16, pretrained=True):
    vit_base = ViT(image_size=image_size, patch_size=16, num_classes=num_classes, channels=3, dim=768, depth=12, heads=12, mlp_dim=3072)
    print("-----------------------------------------------------------")
    # 冻结vit所有层训练
    for param in vit_base.parameters():
        param.requires_grad = False
    if pretrained:
        pretrain_dict = torch.load(model_urls['zlp-vit-base'])
        model_dict = {}
    ...
    model = Deeplabv3plus_vit(backbone=vit_base, num_classes=num_classes, os=os, freeze_bn=False)
    return model


"""print layers and params of network"""
if __name__ == '__main__':
    model = Deeplabv3plus_vitbase(num_classes=2, image_size=128, os=16, pretrained=False)
    # 计算模型的总参数量
