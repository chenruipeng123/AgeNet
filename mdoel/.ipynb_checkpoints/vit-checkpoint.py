...
class ViT(nn.Module):
    ...
    def forward(self, img):
        low_level_features = img
        # 64x64: 输入x经过patch_embedding划分为4*4个patch块，每个块有768个通道
        x = self.to_patch_embedding(img)
        b, n, _ = x.shape
        # low_level_features = self.norm2(x).view(b, -1, 16, 16)
...
#         print(y.shape)
        ...
        x_vis = self.norm2(x).view(b, -1, 16, 16) #（2,768,4,4）
        return x_vis, low_level_features
